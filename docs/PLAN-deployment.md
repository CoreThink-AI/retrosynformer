...
| Modify | `pyproject.toml` — add `[serve]` extra + `rs-serve` CLI |
...
EXPOSE 8080
CMD ["rs-serve", "--host", "0.0.0.0", "--port", "8080", "--workers", "1"]
```
...
6. `pyproject.toml` — `[serve]` + `[cuda]` extras, `rs-serve` script
7. Local smoke test (CPU): `rs-serve --port 8080` + curl aspirin
